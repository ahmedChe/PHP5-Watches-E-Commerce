...
if(isset($_GET['envoie']))
{
	...
	$testm=false;
	$testlog=false;
	while( ($resultat = $res->fetch()) &&($testm==false)&& ($testlog==false))
	{
		...
		{
			?>
			<script language="javascript">alert("Your email is already exist, Just Press login in or choose Another one ")</script>
			<?php
		}
		else
		{
			if ($testlog==1)
				{
					?>
					<script language="javascript">alert("Your login is already exist, Please choose Another one ")</script>
					<?php
				}
				else
				{
